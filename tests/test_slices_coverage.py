"""Coverage gaps in slices.py not hit by test_slices.py / test_slices_pipe.py:
validation guards and operations on a nonexistent slice.
...
def test_set_slice_center_missing_file():
    result = run(
...
def test_set_slice_center_bad_dims(sprite):
...
def test_set_slice_center_nonexistent_slice(sprite):
...
def test_set_slice_pivot_missing_file():
    result = run(slices.set_slice_pivot("/tmp/ase-pytest/nope.aseprite", "s", 0, 0))
    assert "not found" in result


def test_set_slice_pivot_nonexistent_slice(sprite):
...
def test_list_slices_missing_file():
    result = run(slices.list_slices("/tmp/ase-pytest/nope.aseprite"))
    assert "not found" in result


def test_list_slices_corrupt_file():
...
def test_delete_slice_missing_file():
    result = run(slices.delete_slice("/tmp/ase-pytest/nope.aseprite", "s"))
    assert "not found" in result


def test_delete_slice_nonexistent_slice(sprite):
