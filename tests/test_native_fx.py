...
def test_list_convolution_matrices() -> None:
...
def test_apply_convolution_rejects_unknown_matrix(sprite: str) -> None:
    res = run(native_fx.apply_convolution(sprite, "not-a-matrix", "body", 1))
    assert res.startswith("Unknown matrix")


def test_outline_native_inside_colours_the_edge(sprite: str) -> None:
...
def test_invert_colors_is_exact(sprite: str) -> None:
...
def test_adjust_hsl_native(sprite: str) -> None:
    ok(run(native_fx.adjust_hsl_native(sprite, "body", 1, 40, 0, 0)))


def test_adjust_brightness_contrast(sprite: str) -> None:
    ok(run(native_fx.adjust_brightness_contrast(sprite, "body", 1, 30, 10)))


def test_apply_convolution_blur(sprite: str) -> None:
    ok(run(native_fx.apply_convolution(sprite, "blur-3x3", "body", 1)))


def test_adjust_hsl_native_rejects_out_of_range(sprite: str) -> None:
    assert run(native_fx.adjust_hsl_native(sprite, "body", 1, 999)).startswith(
        "hue must be"
    )


def test_extract_palette(sprite: str) -> None:
...
def test_adjust_hsl_native_rejects_saturation_out_of_range(sprite: str) -> None:
    result = run(native_fx.adjust_hsl_native(sprite, "body", 1, 0, 999, 0))
    assert result == "saturation and lightness must be -100..100"


def test_adjust_hsl_native_rejects_lightness_out_of_range(sprite: str) -> None:
    result = run(native_fx.adjust_hsl_native(sprite, "body", 1, 0, 0, 999))
    assert result == "saturation and lightness must be -100..100"


def test_adjust_brightness_contrast_rejects_out_of_range(sprite: str) -> None:
...
def test_extract_palette_rejects_max_colors_out_of_range(sprite: str) -> None:
...
def test_outline_native_reports_layer_not_found(sprite: str) -> None:
    result = run(native_fx.outline_native(sprite, "no-such-layer"))
    assert result.startswith("Failed to outline:")


def test_adjust_hsl_native_reports_layer_not_found(sprite: str) -> None:
    result = run(native_fx.adjust_hsl_native(sprite, "no-such-layer"))
    assert result.startswith("Failed to adjust HSL:")


def test_adjust_brightness_contrast_reports_layer_not_found(sprite: str) -> None:
...
def test_invert_colors_reports_layer_not_found(sprite: str) -> None:
    result = run(native_fx.invert_colors(sprite, "no-such-layer"))
    assert result.startswith("Failed to invert:")


def test_apply_convolution_reports_layer_not_found(sprite: str) -> None:
...
def test_adjust_hsl_native_with_region(sprite: str) -> None:
    ok(run(native_fx.adjust_hsl_native(sprite, "body", 1, 20, 0, 0, 8, 8, 8, 8)))


def test_adjust_brightness_contrast_with_region(sprite: str) -> None:
    ok(run(native_fx.adjust_brightness_contrast(sprite, "body", 1, 10, 5, 8, 8, 8, 8)))


def test_invert_colors_with_region(sprite: str) -> None:
    ok(run(native_fx.invert_colors(sprite, "body", 1, 8, 8, 8, 8)))


def test_apply_convolution_with_region(sprite: str) -> None:
    ok(run(native_fx.apply_convolution(sprite, "blur-3x3", "body", 1, 8, 8, 8, 8)))


def test_extract_palette_with_alpha(sprite: str) -> None:
    result = json.loads(ok(run(native_fx.extract_palette(sprite, 8, with_alpha=True))))
    assert result["count"] >= 1


def test_extract_palette_reports_subprocess_failure(sprite: str) -> None:
    with patch(
