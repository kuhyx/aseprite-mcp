    ...
    ok(
        run(
            drawing.apply_gradient_rect(
                sprite, "grad", 1, 4, 4, 16, 1, "#000000", "#FFFFFF", True, True
            )
        )
    )
    ...
    ok(
        run(
            drawing.draw_pixels_at(
                sprite, "alpha", 1, [{"x": 6, "y": 6, "color": "#FF000080"}], True
            )
        )
    )
...
def test_draw_accepts_short_hex(sprite):
    ...
    ok(
        run(
            drawing.draw_pixels_at(
                sprite, "short", 1, [{"x": 5, "y": 5, "color": "#0F0"}], True
            )
        )
    )
...
def test_draw_rejects_bad_hex(sprite):
    ...
    res = run(
        drawing.draw_pixels_at(
            sprite, "bad", 1, [{"x": 0, "y": 0, "color": "#GG0000"}], True
        )
    )
