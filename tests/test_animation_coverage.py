"""Coverage-gap tests for animation.py: validation paths, branches, and
tools not exercised by test_animation.py or test_error_propagation.py.
...
def test_missing_file_every_tool():
    checks = [
...
def test_set_layer_visibility_true_and_false(sprite):
    ok(run(animation.set_layer_visibility(sprite, "body", False)))
    info = json.loads(run(animation.get_sprite_info(sprite)))
    layer = next(l for l in info["layers"] if l["name"] == "body")
    assert layer["visible"] is False
    ok(run(animation.set_layer_visibility(sprite, "body", True)))


# --- set_layer_opacity ---


def test_set_layer_opacity_success(sprite):
    ok(run(animation.set_layer_opacity(sprite, "body", 128)))


def test_set_layer_opacity_rejects_below_zero(sprite):
    result = run(animation.set_layer_opacity(sprite, "body", -1))
    assert result == "Opacity must be between 0 and 255"


def test_set_layer_opacity_rejects_above_255(sprite):
...
def test_get_sprite_info_reports_group_and_tag(sprite):
    ...
    info = json.loads(run(animation.get_sprite_info(sprite)))
    child = next(l for l in info["layers"] if l["name"] == "child")
...
def test_duplicate_frame_range_success(sprite):
...
def test_set_cel_position_existing_cel(sprite):
    ok(run(animation.set_cel_position(sprite, "body", 1, 4, 4)))


def test_set_cel_position_create_if_missing_no_source(sprite):
...
def test_set_cel_position_create_if_missing_with_source(sprite):
    ok(
        run(
...
def test_set_cel_position_missing_cel_no_create_is_noop(sprite):
...
def test_tween_cel_positions_single_frame_span_zero(sprite):
...
def test_offset_cel_positions_success(sprite):
    ok(run(animation.offset_cel_positions(sprite, "body", 1, 2, 1, -1)))


def test_offset_cel_positions_out_of_bounds(sprite):
    result = run(animation.offset_cel_positions(sprite, "body", 1, 999, 1, 1))
    assert "Frame range out of bounds" in result


def test_offset_cel_positions_missing_layer(sprite):
...
def test_copy_cel_success_replace_true(sprite):
    ok(run(animation.copy_cel(sprite, "body", 1, 2, replace=True)))


def test_copy_cel_success_replace_false_no_existing(sprite):
    ok(run(animation.create_cel(sprite, "body", 6, 0, 0)))
    ok(run(animation.clear_cel(sprite, "body", 6)))
    ok(run(animation.copy_cel(sprite, "body", 1, 6, replace=False)))


def test_copy_cel_replace_false_existing_dst_is_noop(sprite):
...
def test_copy_frame_append_new(sprite):
    before = json.loads(run(animation.get_sprite_info(sprite)))["frames"]
...
def test_copy_frame_explicit_target_overwrite(sprite):
    result = ok(run(animation.copy_frame(sprite, 1, 2, overwrite=True)))
    assert "copied to frame 2" in result


def test_copy_frame_explicit_target_no_overwrite(sprite):
    ok(run(animation.copy_frame(sprite, 1, 3, overwrite=False)))


def test_copy_frame_source_out_of_range(sprite):
    result = run(animation.copy_frame(sprite, 999))
    assert "Source frame out of range" in result


def test_copy_frame_target_out_of_range_is_fabricated_success(sprite):
...
def test_propagate_frame_to_range_success(sprite):
    ok(run(animation.propagate_frame_to_range(sprite, 1, 2, 3, overwrite=True)))


def test_propagate_frame_to_range_no_overwrite(sprite):
    ok(run(animation.propagate_frame_to_range(sprite, 1, 2, 3, overwrite=False)))


def test_propagate_frame_to_range_source_out_of_range(sprite):
    result = run(animation.propagate_frame_to_range(sprite, 999, 1, 2))
    assert "Source frame out of range" in result


def test_propagate_frame_to_range_bounds_out_of_range(sprite):
...
def test_set_tag_all_directions(sprite):
...
def test_set_tag_updates_existing_tag(sprite):
    ...
    assert tag["from"] == 2 and tag["to"] == 3 and tag["direction"] == "reverse"


def test_set_tag_out_of_bounds(sprite):
...
def test_set_onion_skin_success(sprite):
    result = ok(run(animation.set_onion_skin(sprite, True, 2, 2, 128)))
    assert "UI-only in batch mode" in result


def test_set_onion_skin_rejects_negative_before(sprite):
    result = run(animation.set_onion_skin(sprite, True, -1, 2, 128))
    assert result == "Before/after must be >= 0"


def test_set_onion_skin_rejects_negative_after(sprite):
    result = run(animation.set_onion_skin(sprite, True, 2, -1, 128))
    assert result == "Before/after must be >= 0"


def test_set_onion_skin_rejects_opacity_below_zero(sprite):
    result = run(animation.set_onion_skin(sprite, True, 2, 2, -1))
    assert result == "Opacity must be between 0 and 255"


def test_set_onion_skin_rejects_opacity_above_255(sprite):
    result = run(animation.set_onion_skin(sprite, True, 2, 2, 256))
    assert result == "Opacity must be between 0 and 255"


def test_set_onion_skin_disabled(sprite):
    result = ok(run(animation.set_onion_skin(sprite, False)))
...
def test_tween_cel_positions_eased_all_modes(sprite):
...
def test_tween_cel_positions_eased_default_via_empty_string(sprite):
...
def test_tween_cel_positions_eased_normalizes_case_and_whitespace(sprite):
    ok(
        run(
            animation.tween_cel_positions_eased(
...
def test_tween_cel_positions_eased_rejects_unsupported(sprite):
    result = run(
        animation.tween_cel_positions_eased(
...
def test_tween_cel_positions_eased_out_of_bounds(sprite):
    result = run(
...
def test_tween_cel_positions_eased_missing_layer(sprite):
    result = run(animation.tween_cel_positions_eased(sprite, "NOPE", 1, 2, 0, 0, 2, 2))
    assert "Layer not found" in result


def test_tween_cel_positions_eased_create_missing_with_source(sprite):
    ok(
        run(
            animation.tween_cel_positions_eased(
                sprite,
...
def test_tween_cel_positions_eased_single_frame_span_zero(sprite):
...
def test_oscillate_cel_positions_success(sprite):
    ok(
        run(
...
def test_oscillate_cel_positions_out_of_bounds(sprite):
    result = run(animation.oscillate_cel_positions(sprite, "body", 1, 999))
    assert "Frame range out of bounds" in result


def test_oscillate_cel_positions_missing_layer(sprite):
    result = run(animation.oscillate_cel_positions(sprite, "NOPE", 1, 2))
    assert "Layer not found" in result


def test_oscillate_cel_positions_create_missing_with_source(sprite):
    ok(
        run(
            animation.oscillate_cel_positions(
                sprite,
...
def test_oscillate_cel_positions_single_frame_span_zero(sprite):
...
def test_tween_cel_opacity_eased_success(sprite):
    ok(run(animation.tween_cel_opacity_eased(sprite, "body", 1, 3, 255, 0)))


def test_tween_cel_opacity_eased_rejects_start_below_zero(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "body", 1, 2, -1, 255))
    assert result == "Opacity must be between 0 and 255"


def test_tween_cel_opacity_eased_rejects_start_above_255(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "body", 1, 2, 256, 0))
    assert result == "Opacity must be between 0 and 255"


def test_tween_cel_opacity_eased_rejects_end_below_zero(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "body", 1, 2, 255, -1))
    assert result == "Opacity must be between 0 and 255"


def test_tween_cel_opacity_eased_rejects_end_above_255(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "body", 1, 2, 0, 256))
    assert result == "Opacity must be between 0 and 255"


def test_tween_cel_opacity_eased_rejects_unsupported_easing(sprite):
    result = run(
...
def test_tween_cel_opacity_eased_out_of_bounds(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "body", 1, 999, 0, 255))
    assert "Frame range out of bounds" in result


def test_tween_cel_opacity_eased_missing_layer(sprite):
    result = run(animation.tween_cel_opacity_eased(sprite, "NOPE", 1, 2, 0, 255))
    assert "Layer not found" in result


def test_tween_cel_opacity_eased_create_missing_with_source(sprite):
    ok(
        run(
...
def test_tween_cel_opacity_eased_single_frame_span_zero(sprite):
    ok(run(animation.tween_cel_opacity_eased(sprite, "body", 1, 1, 0, 255)))


def test_tween_cel_opacity_eased_all_easing_modes(sprite):
    for easing in ("linear", "ease_in", "ease_out", "ease_in_out", "smoothstep"):
        ok(
            run(
                animation.tween_cel_opacity_eased(
...
def test_tween_cel_scale_eased_success_center_anchor(sprite):
    ok(run(animation.tween_cel_scale_eased(sprite, "body", 1, 2, 1.0, 2.0)))


def test_tween_cel_scale_eased_topleft_anchor(sprite):
    ok(
        run(
...
def test_tween_cel_scale_eased_rejects_start_scale_zero(sprite):
    result = run(animation.tween_cel_scale_eased(sprite, "body", 1, 2, 0, 2.0))
    assert result == "Scale must be > 0"


def test_tween_cel_scale_eased_rejects_end_scale_negative(sprite):
    result = run(animation.tween_cel_scale_eased(sprite, "body", 1, 2, 1.0, -1))
    assert result == "Scale must be > 0"


def test_tween_cel_scale_eased_rejects_unsupported_easing(sprite):
    result = run(
...
def test_tween_cel_scale_eased_default_easing_via_empty_string(sprite):
    ok(run(animation.tween_cel_scale_eased(sprite, "body", 1, 2, 1.0, 2.0, easing="")))


def test_tween_cel_scale_eased_rejects_unsupported_anchor(sprite):
    result = run(
        animation.tween_cel_scale_eased(
...
def test_tween_cel_scale_eased_default_anchor_via_empty_string(sprite):
    ok(run(animation.tween_cel_scale_eased(sprite, "body", 1, 2, 1.0, 2.0, anchor="")))


def test_tween_cel_scale_eased_out_of_bounds(sprite):
    result = run(animation.tween_cel_scale_eased(sprite, "body", 1, 999, 1.0, 2.0))
    assert "Frame range out of bounds" in result


def test_tween_cel_scale_eased_missing_layer(sprite):
    result = run(animation.tween_cel_scale_eased(sprite, "NOPE", 1, 2, 1.0, 2.0))
    assert "Layer not found" in result


def test_tween_cel_scale_eased_explicit_source_frame(sprite):
    ok(
        run(
            animation.tween_cel_scale_eased(
...
def test_tween_cel_scale_eased_source_frame_out_of_range(sprite):
    result = run(
        animation.tween_cel_scale_eased(
...
def test_tween_cel_scale_eased_no_source_cel(sprite):
...
def test_tween_cel_scale_eased_no_replace_keeps_existing(sprite):
    ok(
        run(
            animation.tween_cel_scale_eased(
...
def test_tween_cel_scale_eased_no_create_missing(sprite):
...
def test_delete_frame_success(sprite):
    before = json.loads(run(animation.get_sprite_info(sprite)))["frames"]
...
def test_delete_frame_out_of_range(sprite):
    result = run(animation.delete_frame(sprite, 999))
    assert "Frame index out of range" in result


def test_delete_frame_rejects_only_frame(base_dir):
...
def test_delete_tag_success(sprite):
...
def test_delete_tag_not_found(sprite):
...
def test_set_cel_opacity_success(sprite):
    ok(run(animation.set_cel_opacity(sprite, "body", 1, 200)))


def test_set_cel_opacity_rejects_below_zero(sprite):
    result = run(animation.set_cel_opacity(sprite, "body", 1, -1))
    assert result == "Opacity must be between 0 and 255"


def test_set_cel_opacity_rejects_above_255(sprite):
    result = run(animation.set_cel_opacity(sprite, "body", 1, 256))
    assert result == "Opacity must be between 0 and 255"


def test_set_cel_opacity_out_of_range_frame(sprite):
    result = run(animation.set_cel_opacity(sprite, "body", 999, 100))
    assert "Frame index out of range" in result


def test_set_cel_opacity_missing_layer(sprite):
    result = run(animation.set_cel_opacity(sprite, "NOPE", 1, 100))
    assert "Layer not found" in result


def test_set_cel_opacity_no_cel_at_layer_frame(sprite):
