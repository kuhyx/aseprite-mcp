...
def test_render_onion_skin(sprite: str) -> None:
...
def test_compare_identical_frames(sprite: str) -> None:
...
def test_compare_differing_frames(sprite: str) -> None:
    ok(
        run(
            drawing.draw_rectangle_at(
                sprite, "body", 3, 0, 0, 4, 4, "#FFFFFF", fill=True
            )
        )
    )
...
def test_get_color_stats(sprite: str) -> None:
