"""Coverage gaps in transform.py.

Covers rotate_layer, resize_canvas, crop_canvas success paths and
validation guards not hit by test_error_propagation.py.
...
def fresh(request: pytest.FixtureRequest) -> str:
    """Build a small non-square sprite with an asymmetric painted layer.
    ...
    ok(
        run(
            drawing.draw_rectangle_at(path, "body", 1, 0, 0, 4, 2, "#D04648", fill=True)
        )
    )
    return path


def _opaque_count(path: str, frame: int = 1) -> int:
...
def test_rotate_layer_missing_file() -> None:
    result = run(
...
def test_resize_canvas_corrupt_file() -> None:
...
def test_crop_canvas_missing_file() -> None:
    result = run(
...
def test_crop_canvas_bad_dims(fresh: str) -> None:
    result = run(transform.crop_canvas(fresh, 0, 0, 4, 0))
    assert "must be > 0" in result


def test_crop_canvas_success(fresh: str) -> None:
