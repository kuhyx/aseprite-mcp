...
def test_copy_region_reports_source_frame_out_of_range(sprite):
    result = run(selection.copy_region(sprite, "body", 99, 0, 0, 4, 4, 0, 0))
    assert result.startswith("Failed to copy region:")
    assert "Source frame index out of range" in result


def test_copy_region_reports_target_frame_out_of_range(sprite):
    result = run(
        selection.copy_region(
...
def test_copy_region_to_explicit_target_layer_and_frame(sprite):
