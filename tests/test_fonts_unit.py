...
requires_system_ttf = pytest.mark.skipif(
    not Path(SYSTEM_TTF).exists(), reason="no Liberation Sans on this machine"
)


@pytest.fixture(autouse=True)
def _clean_font_cache() -> Iterator[None]:
    """Clear the module-global `_loaded` cache before and after each test."""
    fontlib.clear_cache()
    yield
    fontlib.clear_cache()


def _write_font_json(directory: Path, spec: dict[str, Any]) -> None:
    with (Path(directory) / "font.json").open("w", encoding="utf-8") as fh:
        fh.write(json.dumps(spec))


def _blank_sheet(
    directory: Path, filename: str, size: tuple[int, int] = (4, 4)
) -> None:
    Image.new("RGBA", size, (0, 0, 0, 0)).save(Path(directory) / filename)
...
def test_malformed_json_raises_font_error(tmp_path: Path) -> None:
...
def test_missing_font_json_raises_font_error(tmp_path: Path) -> None:
    with pytest.raises(fontlib.FontError, match="Could not read"):
        fontlib.BitmapFont(str(tmp_path))


def test_no_sheets_key_raises_font_error(tmp_path: Path) -> None:
    _write_font_json(tmp_path, {"name": "x"})
    with pytest.raises(fontlib.FontError, match="declares no sheets"):
        fontlib.BitmapFont(str(tmp_path))


def test_empty_sheets_list_raises_font_error(tmp_path: Path) -> None:
    _write_font_json(tmp_path, {"name": "x", "sheets": []})
    with pytest.raises(fontlib.FontError, match="declares no sheets"):
        fontlib.BitmapFont(str(tmp_path))


def test_missing_sheet_png_raises_font_error(tmp_path: Path) -> None:
    _write_font_json(
        tmp_path,
        {
            "name": "x",
...
def test_sheet_missing_file_key_raises_font_error(tmp_path: Path) -> None:
    _write_font_json(
        tmp_path,
        {
            "name": "x",
...
def _simple_bitmap_font(
    tmp_path: Path,
    chars: str = "A",
    extra_sheet_kwargs: dict[str, Any] | None = None,
) -> fontlib.BitmapFont:
...
def test_glyph_returns_none_for_unmapped_codepoint(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path)
    assert font.glyph(ord("Z")) is None


def test_glyph_cache_hits_for_both_present_and_absent(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path)
...
def test_null_codepoint_in_chars_row_is_not_indexed(tmp_path: Path) -> None:
...
def test_first_sheet_wins_for_duplicate_codepoint(tmp_path: Path) -> None:
...
def test_dark_ink_rule_reads_box_and_ink(tmp_path: Path) -> None:
    """White (255,255,255,255) background box, near-black ink inside it.

    Ink touches the cell's leftmost column so box_h detection works.
    """
    ...
    assert (0, 0) in glyph.ink
    assert (2, 3) in glyph.ink
    assert (3, 0) not in glyph.ink  # outside the ink region


def test_dark_ink_rule_with_advance_ink(tmp_path: Path) -> None:
    img = Image.new("RGBA", (6, 6), (255, 255, 255, 255))
    px = img.load()
    for y in range(4):
        for x in range(3):
...
def test_alpha_ink_rule_blank_cell_advance_box(tmp_path: Path) -> None:
    """Cover advance='box' on an all-transparent (blank) cell.

    box_w is the full cell width (truthy), so the space_width fallback is
    NOT used.
    """
    _blank_sheet(tmp_path, "s.png")
    _write_font_json(
...
def test_alpha_ink_rule_blank_cell_advance_ink_uses_space_width(tmp_path: Path) -> None:
    """Cover advance='ink' on a blank cell.

    ink_width is 0 (falsy), so the space_width + letter_gap fallback
    applies.
    """
    _blank_sheet(tmp_path, "s.png")
    _write_font_json(
...
# ---------------------------------------------------------------------------
# Section: tests for BitmapFont's layout method
# ---------------------------------------------------------------------------


def test_layout_empty_string(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path)
...
def test_layout_unknown_chars_contribute_nothing(tmp_path: Path) -> None:
...
def test_layout_scale_multiplies_ink_and_advance(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path, chars="A")
...
def test_layout_letter_spacing_is_additive_per_glyph(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path, chars="A")
...
# ---------------------------------------------------------------------------
# Section: tests for glyph_from_rows overrides
# ---------------------------------------------------------------------------


def test_glyph_from_rows_explicit_ascent() -> None:
...
def test_glyph_from_rows_default_ascent_is_row_count() -> None:
    glyph = fontlib._glyph_from_rows(["##", ".#", ".."], ascent=None, letter_gap=1)
    assert glyph.ascent == 3


def test_glyph_from_rows_treats_space_as_empty() -> None:
    glyph = fontlib._glyph_from_rows(["# #"], ascent=1, letter_gap=0)
    assert glyph.ink == {(0, 0), (2, 0)}


def test_overrides_are_indexed_by_codepoint(tmp_path: Path) -> None:
...
@requires_system_ttf
def test_truetype_name_is_derived_from_filename() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
    assert font.name == "LiberationSans-Regular"


@requires_system_ttf
def test_truetype_load_error_wraps_in_font_error() -> None:
...
@requires_system_ttf
def test_truetype_load_error_on_non_font_file(tmp_path: Path) -> None:
...
@requires_system_ttf
def test_truetype_layout_empty_string() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
@requires_system_ttf
def test_truetype_letter_spacing_widens_advance() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
@requires_system_ttf
def test_truetype_antialias_produces_more_ink_than_hard_threshold() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
@requires_system_ttf
def test_truetype_threshold_changes_ink_count() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
@requires_system_ttf
def test_truetype_layout_with_and_without_letter_spacing_agree_on_single_char() -> None:
    """Cover agreement between the fast path and per-char loop.
...
# ---------------------------------------------------------------------------
# Section: discovery — _iter_user_fonts / _iter_system_fonts / available_fonts
# ---------------------------------------------------------------------------


@pytest.fixture
def isolated_font_dirs(monkeypatch: pytest.MonkeyPatch) -> Iterator[str]:
    """Point FONT_DIR/_SYSTEM_FONT_DIRS at throwaway directories.

    Keeps discovery tests independent of this machine's real
    ~/.aseprite-mcp/fonts or top-level system font layout.
    """
...
def test_iter_user_fonts_missing_dir_yields_nothing(
...
def test_iter_user_fonts_skips_dirs_without_font_json_and_stray_files(
    isolated_font_dirs: str,
) -> None:
    user_dir = isolated_font_dirs
    (Path(user_dir) / "not_a_font_dir").mkdir(parents=True)
    with (Path(user_dir) / "readme.txt").open("w") as fh:
        fh.write("hi")
    assert list(fontlib._iter_user_fonts()) == []


@requires_system_ttf
def test_iter_user_fonts_finds_bitmap_dir_and_ttf_file(
    isolated_font_dirs: str,
) -> None:
    user_dir = isolated_font_dirs
    bmp_dir = Path(user_dir) / "mybitmap"
    ...
    shutil.copy(SYSTEM_TTF, Path(user_dir) / "MyTTF.ttf")
...
def test_iter_system_fonts_skips_unreadable_dir(
    ...
    def raising_iterdir(_self: Path) -> Iterator[Path]:
        msg = "denied"
        raise OSError(msg)
...
@requires_system_ttf
def test_available_fonts_dedup_is_case_insensitive_and_user_first(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    user_dir = tempfile.mkdtemp()
    sys_dir = tempfile.mkdtemp()
    try:
        shutil.copy(SYSTEM_TTF, Path(user_dir) / "Dup.ttf")
        shutil.copy(SYSTEM_TTF, Path(sys_dir) / "dup.ttf")
        monkeypatch.setattr(fontlib, "FONT_DIR", user_dir)
        ...
        assert matches[0]["path"] == str(Path(user_dir) / "Dup.ttf")
    finally:
        shutil.rmtree(user_dir, ignore_errors=True)
...
# ---------------------------------------------------------------------------
# Section: tests for load_font and clear_cache
# ---------------------------------------------------------------------------


@pytest.mark.usefixtures("isolated_font_dirs")
...
def test_load_font_name_lookup_iterates_without_matching(
    isolated_font_dirs: str, tmp_path: Path
) -> None:
    ...
    user_dir = isolated_font_dirs
    font_dir = Path(user_dir) / "somebitmap"
    font_dir.mkdir(parents=True)
    _blank_sheet(tmp_path, "s.png")
    shutil.copy(tmp_path / "s.png", font_dir / "s.png")
    with (font_dir / "font.json").open("w", encoding="utf-8") as f:
        json.dump(
            {
                "name": "somebitmap",
                "sheets": [
...
def test_load_font_path_like_spec_that_does_not_exist_falls_back_to_name_lookup() -> (
    ...
    """Cover a spec containing a path separator that doesn't resolve.

    It's first tried as an expanded path; if that path doesn't resolve to
    a bitmap dir or a file, load_font falls through to the by-name search
    (which also fails here) rather than raising a different,
    path-specific error.
    """
    with pytest.raises(fontlib.FontError, match="not found"):
...
def test_load_font_dispatches_bitmap_by_path(tmp_path: Path) -> None:
    _blank_sheet(tmp_path, "s.png")
    _write_font_json(
        tmp_path,
        {
...
@requires_system_ttf
def test_load_font_dispatches_truetype_by_path() -> None:
    font = fontlib.load_font(SYSTEM_TTF)
    assert isinstance(font, fontlib.TrueTypeFont)


@requires_system_ttf
def test_load_font_dispatches_by_name_lookup(isolated_font_dirs: str) -> None:
    user_dir = isolated_font_dirs
    shutil.copy(SYSTEM_TTF, Path(user_dir) / "NamedFont.ttf")
    font = fontlib.load_font("NamedFont")
    assert isinstance(font, fontlib.TrueTypeFont)


def test_load_font_caches_by_spec_string(tmp_path: Path) -> None:
    _blank_sheet(tmp_path, "s.png")
    _write_font_json(
        tmp_path,
        {
...
# ---------------------------------------------------------------------------
# Section: tests for the dilate helper
# ---------------------------------------------------------------------------


def test_dilate_zero_passes_is_identity() -> None:
    assert fontlib._dilate({(0, 0)}, 0) == {(0, 0)}


def test_dilate_one_pass_grows_right_and_down() -> None:
    assert fontlib._dilate({(0, 0)}, 1) == {(0, 0), (1, 0), (0, 1)}


def test_dilate_two_passes_compounds() -> None:
...
# ---------------------------------------------------------------------------
# Section: tests for the shape function
# ---------------------------------------------------------------------------


def test_shape_rejects_negative_bold(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path)
    with pytest.raises(fontlib.FontError, match="bold must be >= 0"):
        fontlib.shape("A", font, size=1, bold=-1)


def test_shape_rejects_zero_size_for_bitmap_font(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path)
...
@requires_system_ttf
def test_shape_rejects_zero_size_for_truetype_font() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
@requires_system_ttf
def test_shape_truetype_font_success() -> None:
    font = fontlib.TrueTypeFont(SYSTEM_TTF)
...
def test_shape_empty_ink_returns_zeroed_metrics(tmp_path: Path) -> None:
    """Cover the early-return metrics branch when every char is unmapped.

    No ink means no ink -> the early-return metrics branch, which (unlike
    the non-empty branch) reports advance_width without the `+ bold` term.
    """
    font = _simple_bitmap_font(tmp_path, chars="A")
...
def test_shape_bold_adds_dilation_and_widens_advance(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path, chars="A")
...
def test_shape_metrics_bounding_box(tmp_path: Path) -> None:
    font = _simple_bitmap_font(tmp_path, chars="A")
